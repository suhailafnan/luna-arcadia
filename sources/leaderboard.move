...
public fun get_points_10h_for_testing(account_addr: address): vector<PlayerScore> acquires LeaderboardData {
  ...
  let old_points = &lb.points_10h;
  let len = vector::length(old_points);
  let new_points = vector::empty<PlayerScore>();
  let i = 0;
  while (i < len) {
    let entry_copy = *vector::borrow(old_points, i);
    vector::push_back(&mut new_points, entry_copy);
    i = i + 1;
  };
  new_points
}
  ...
  // public fun get_points_10h_for_testing(addr: address): vector<PlayerScore> acquires LeaderboardData {
  //   let lb = borrow_global<LeaderboardData>(addr);
  //   let old_points = &lb.points_10h;
  //   let new_points = vector::empty<PlayerScore>();
  //   let len = vector::length(old_points);
  //   let i = 0;
  ...
  //     public fun get_points_10h_for_testing(addr: address): vector<PlayerPoints> acquires LeaderboardData {
  //   let lb = borrow_global<LeaderboardData>(addr);
  //   let old_points = &lb.points_10h;
  //   let len = vector::length(old_points);
  //   let new_points = vector::empty<PlayerPoints>();
  //   let i = 0;
  //   while (i < len) {
  //     let entry = *vector::borrow(old_points, i);
  //     vector::push_back(&mut new_points, entry);
  //     i = i + 1;
  //   };
  //   new_points
  // }
